...
// функция длинная, потому что универсальная для количества знаков для выигрыша и размера доски
export default function checkWin(
  ...
  for (let i = rowIndex - 1; i >= 0; i--) {
    if (boardMatrix[i][columnIndex].sign === move) {
      sumVertical++;
    } else break;
